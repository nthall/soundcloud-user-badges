badgeTemplate = Handlebars.compile([
  // container
  "<div class='previewBadge sc-border-dark sc-clearfix' style='position:absolute; display:block; background-color:white; width:400px; overflow:hidden; z-index:101;'>",
    // top section
    "<div class='sc-border-light-bottom sc-clearfix'>",
      "<div class='userImageBadge userBadge__avatar sc-media-image large' style='float:left;'>",
        "<a href='/{{permalink}}'>",
          "<div class='image image__hasPlaceholder image__hasPlaceholder-70 image__hasPlaceholder-user' style='height:70px; width:70px'>",
            "<img src='{{avatar_url}}' class='image__full g-opacity-transition' width='70' height='70'>",
            "{{#if online}}",
              "<span class='sc-icon-online sc-icon-small userImageBadge__online'></span>",
            "{{/if}}",
          "</div>",
        "</a>",
      "</div>",
      "<div style='float:left; display:block;'>",
        "<h3 class='userBadge__Title sc-font'>",
          "<a href='/{{permalink}}' class='sc-link-dark'>{{username}}</a>",
        "</h3>",
        "<ul class='stats sc-list-nostyle sc-clearfix userStats stats__large' style='margin-bottom:5px'>",
          "<li class='stats__item'>",
            "<a href='/{{permalink}}'>",
              "<span class='sc-ministats-icon-sounds sc-ministats-icon-large sc-border-right-dark sc-ir'>Sounds:</span>",
              "{{track_count}}",
            "</a>",
          "</li>",
          "<li class='stats__item'>",
            "<a href='/{{permalink}}/sets'>",
              "<span class='sc-icon-set-dark sc-icon-large sc-border-right-dark sc-ir'>Sets:</span>",
              "{{playlist_count}}",
            "</a>",
          "</li>",
          "<li class='stats__item'>",
            "<a href='/{{permalink}}/likes'>",
              "<span class='sc-icon-like-dark sc-icon-large sc-border-right-dark sc-ir'>Likes:</span>",
              "{{public_favorites_count}}",
            "</a>",
          "</li>",
        "</ul>",
        "<ul class='stats sc-list-nostyle sc-clearfix userStats stats__large' style='margin-bottom:5px'>",
          "<li class='stats__item'>",
            "<a href='/{{permalink}}/following'>",
              "<span class='sc-icon-following-dark sc-icon-large sc-border-right-dark sc-ir'>Followings:</span>",
              "{{followings_count}}",
            "</a>",
          "</li>",
          "<li class='stats__item'>",
            "<a href='/{{permalink}}/followers'>",
              "<span class='sc-ministats-icon-followers sc-ministats-icon-large sc-border-right-dark sc-ir'>Followers:</span>",
              "{{followers_count}}",
            "</a>",
          "</li>",
          "<li class='stats__item'>",
            "<a href='/{{permalink}}/comments'>",
              "<span class='sc-icon-comment-dark sc-icon-large sc-border-right-dark sc-ir'>Comments:</span>",
            "</a>",
          "</li>",
        "</ul>",
      "</div>",
    "</div>",
    "<div class='sc-clearfix'>",
      "<span class='userInfo__details sc-text-light sc-border-light-bottom'>",
        "{{full_name}} - {{city}}, {{country}}",
      "</span>",
      "<span class='userInfo__details sc-text-light' style='display:block; overflow:hidden; text-overflow:ellipsis; height:36px'>",
        "{{description}}",
      "</span>",
    "</div>",
  "</div>"].join('\n'));

//todo: more info???
/////// fix dimensions - this is probably a late-stage polish once contents are set.
/////// full_name & loc span needs logic/helpers to deal with missing vals.
/////// add truncation of some kind to description.
